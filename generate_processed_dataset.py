...
for seq in sampled_seqs:
    new_gc_content.append(seq[1])

ax[1].hist(new_gc_content, bins=200, alpha=0.5, label="Sampled MM", density=True)
ax[1].hist([s[1] for s in other_sequences], bins=200, alpha=0.5, label="Other", density=True)
...
print("Number of val sequences: ", len(list(set([d[0] for d in dataset]))))
...
print("Total number of sequences: ", len(val_data) + len(train_data), " = ", len(dataset))
...
val_dataset_padded = []
test_dataset_padded = []
# for j in tqdm(range(-10, 10)):
for j in tqdm(range(-3, 3)):
# for j in [1]:
...
optimize = torch.optim.Adam(model.parameters(), lr=LR, weight_decay=WD)
...
for epoch in tqdm(range(N_EPOCHS)):
    ...
    for batch in train_loader:
        ...
        loss.backward()
        optimize.step()
        running_corrects, running_total = calc_accuracy(labels, outputs, running_corrects, running_total)
    acc = running_corrects / running_total
    print(f"Epoch [{epoch+1}/{N_EPOCHS}], Loss: {loss.item():.4f}, Accuracy: {acc:.4f}")
    history["train_loss"].append(loss.item()/len(train_loader))
    ...
    for batch in val_loader:
        ...
        TP, TN, FP, FN = update_confusion_matrix(labels, outputs, TP, TN, FP, FN)

    acc = running_corrects / running_total
    print(f"Val Accuracy: {acc:.4f}")
    print(f"TP: {TP}, FP: {FP}\nFN: {FN}, TN: {TN}")
    history["val_loss"].append(loss.item()/len(val_loader))
...
minute = now.minute
second = now.second
savetime = f"{day}{hour}{minute*60+second}"

model_name = f"{savetime}_{LABEL}_C{COVERAGE}_{ENH}"
torch.save(model.state_dict(), os.path.join(SAVE_DIR, "models", model_name + ".pth"))
