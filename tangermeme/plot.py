...
def plot_heatmap(X, ax=None, cmap='magma', cbar=True, cbar_label=None):
	if ax is None:
		fig, ax = plt.subplots(figsize=(len(X[0])//15, len(X)))
		show_plot = True
	else:
		show_plot = False

	im = ax.imshow(X, cmap=cmap)
	# set max and min values to be the same
	im.set_clim(-numpy.abs(X).max(), numpy.abs(X).max())
...
def plot_logo(X_attr, ax=None, color=None, annotations=None, start=None, end=None, 
	...
	if ax is None:
		fig, ax = plt.subplots(figsize=(len(X_attr[0])//15, 3))
		show_plot = True
	else:
		show_plot = False
