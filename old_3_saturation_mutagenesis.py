...
fig, ax = plt.subplots(3, 1, figsize=(len(y)//15, 7), sharex=True, dpi=250)
...
max_val = np.max(np.abs(y - wt_out))
ax[0].imshow((y - wt_out).T, cmap="coolwarm", aspect="auto", interpolation="none", vmin=-max_val, vmax=max_val)
...
ax[1].set_ylabel("Change in output\n(arbitrary units)")

# plot_logo((np.log(y / wt_out)).T, ax=ax[0])

# wt_atts = ((((y - wt_out).T)**2).sum(0))**0.5
wt_atts = (np.abs(((y - wt_out).T)).sum(0))
wt_atts = - enhancer[0].numpy() * wt_atts * np.sign((y - wt_out).T.sum(0))

plot_logo(wt_atts, ax=ax[2])
xticks = np.arange(0, len(y), step=25)
ax[2].set_xticks(xticks, xticks - 25)
ax[2].set_xlabel("Position")
ax[2].set_ylabel("Attributions (a. u.)")

plt.tight_layout()
plt.savefig(f"explainability_final/{ENHANCER_NAME}_{model_id}_saturation.png")
plt.close()
