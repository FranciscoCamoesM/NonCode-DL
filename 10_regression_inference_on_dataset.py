...
from regression_training_funcs import CustomDataset
from scipy.stats import pearsonr, spearmanr
import os

model_id = "2026-04-23_11-45-53"
training_dataset = [15, "E25E102"]
inference_dataset = [15, "E25E10"]
AA = 1
BB = 0.45/.95
...
REMOVE_TRAINING_SEQS_FROM_INFERENCE = True
...
inference_data = get_interpolated_seq_label_pairs(coverage=inference_coverage, label_values=lv, enh_name=inference_enh, local_path=os.path.join("../NonCode", get_dataloc(inference_enh)), BIN_CELLS=inference_bin_cells)
...
if REMOVE_TRAINING_SEQS_FROM_INFERENCE:
    ...
    training_enh_data_dict = get_interpolated_seq_label_pairs(coverage=training_coverage, label_values=lv, enh_name=training_enh, local_path=os.path.join("../NonCode", get_dataloc(training_enh)), BIN_CELLS=training_bin_cells)
...
inference_dataset = CustomDataset(inference_data_filtered)
...
with torch.no_grad():
    for seqs, labels in inference_loader:
        seqs = seqs.float()  # Ensure the input is of type float
        seqs = seqs.to(device)
        outputs = model(seqs)
        predictions.extend(outputs.cpu().numpy().flatten().tolist())
        true_values.extend(labels.cpu().numpy().flatten().tolist())
...
# save as csv, with columns "Sequence", "True Value", "Predicted Value"
import pandas as pd
results_df = pd.DataFrame({"Sequence": [seq for seq, _ in inference_data_filtered], "True Value": true_values, "Predicted Value": predictions})
results_df.to_csv(os.path.join(save_dir, f"inference_results_{model_id}_({training_enh}_to_{inference_enh}).csv"), index=False)
print(f"Inference results saved as {os.path.join(save_dir, f'inference_results_{model_id}_({training_enh}_to_{inference_enh}).csv')}")
